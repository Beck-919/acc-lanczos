...
#ifdef COMPLEX3D
template<typename T>
Complex3D<T> typed_conj(const Complex3D<T>& val);
#endif
...
#ifdef COMPLEX3D
template <typename T>
inline std::complex<T> inner_prod(const std::vector<Complex3D<T>>& v1, const std::vector<Complex3D<T>>& v2) {
  assert(v1.size() == v2.size());
  return std::inner_product(std::begin(v1), std::end(v1),
                            std::begin(v2), std::complex<T>(),
                            [](const std::complex<T>& v, const std::complex<T>& u) -> std::complex<T> { return v+u; },
                            [](const Complex3D<T>& a, const Complex3D<T>& b) -> std::complex<T> { return typed_conj(a)*b; });
}
#endif
...
#ifdef COMPLEX3D
template <typename ForwardIterator, typename T>
inline void schmidt_orth(std::vector<Complex3D<T>>& uorth,
                         ForwardIterator first,
                         ForwardIterator last) {
  const auto n = uorth.size();
  ...
  for(auto iter = first; iter != last; ++iter) {
    const auto& uk = *iter;
    std::complex<T> innprod = util::inner_prod(uk, uorth);
