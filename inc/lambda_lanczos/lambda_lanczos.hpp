...
#ifdef COMPLEX3D
template <typename T>
struct VectorRandomInitializer<Complex3D<T>> {
public:
  static void init(std::vector<Complex3D<T>>& v) {
    std::random_device dev;
    std::mt19937 mt(dev());
    std::uniform_real_distribution<T> rand((T)(-1.0), (T)(1.0));
    ...
    size_t n = v.size();
    for(size_t i = 0; i < n; ++i) {
      Complex3D<T> 	val	{std::complex<T>(rand(mt), rand(mt)),
      				std::complex<T>(rand(mt), rand(mt)),
      				std::complex<T>(rand(mt), rand(mt))};
...
public:
  ...
  LambdaLanczos(std::function<void(const std::vector<T>&, std::vector<T>&)> mv_mul, size_t matrix_size, size_t max_iters, bool find_maximum = false)
    : mv_mul(mv_mul),matrix_size(matrix_size), max_iteration(max_iters), find_maximum(find_maximum){}
...
#ifdef PARALLEL_1
    std::cout<<"\nRunning Lanczos with accelerated mat-vec product...\n";
#endif
#ifdef ACC_ALG
    std::cout<<"\nRunning Lanczos in accelerated mode...\n";
    return acc_run(eigvalues, eigvecs);
#endif
  ...
  size_t acc_run(std::vector<real_t<T>>& eigvalues, std::vector<std::vector<T>>& eigvecs);
